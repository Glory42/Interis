          ...
          {visibleSlots.map((slot, index) => (
            <div key={`top-film-slot-${index + 1}`} className="space-y-2">
              <button
                ...
                className="group relative block w-full overflow-hidden rounded-xl border border-border/70 bg-background/30"
              >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                {slot ? (
                  <button
                    ...
                    disabled={isInteractionDisabled}
                    className="inline-flex h-5 w-5 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-secondary/70 hover:text-foreground"
